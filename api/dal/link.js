...
class Link {
     ...
     * @param {string} state Indicates if the link is active or expired.
     */
    constructor(target, action, shortUrl, userId, expiry, state) {
    ...
    static CreateFrom(linkObj) {
        let { UserId, Target, Action, ShortUrl, Expiry, State } = linkObj;
        let newLink = new Link(Target, Action, ShortUrl, UserId, Expiry, State);
